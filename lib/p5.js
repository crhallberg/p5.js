...
amdclean['helpers'] = function (require, core) {
  ...
  var languageStrings = {
      'en': {
        'wrong_param_count': 'YOU WROTE __FUNC__(__GIVEN_X__). ' + '__FUNC__ WAS EXPECTING __PARAMCOUNT__ PARAMETERS. ' + 'TRY __FUNC__(__CORRECT_X__).',
        'empty_var': 'IT LOOKS LIKE __FUNC__ RECEIVED AN EMPTY VARIABLE IN ' + 'SPOT #__SPOT__. THIS COULD BE A PROBLEM WITH VARIABLE SCOPE.',
        'wrong_type': '__FUNC__ WAS EXPECTING A __TYPE__ FOR PARAMETER ' + '#__SPOT__, RECEIVED __VAR__ INSTEAD.',
        'multi_format': ' __FUNC__ TAKES DIFFERENT NUMBERS OF PARAMETERS ' + 'DEPENDING ON WHAT YOU WANT TO DO. CLICK THIS LINK TO LEARN MORE: '
      },
      'bacon': {
        'wrong_param_count': 'You wrote __FUNC__(__GIVEN_X__). ' + '__FUNC__ was expecting __PARAMCOUNT__ parameters. ' + 'Try __FUNC__(__CORRECT_X__).',
        'empty_var': 'It looks like __FUNC__ received an empty variable in ' + 'spot #__SPOT__. This could be a problem with variable scope.',
  ...
  p5.prototype._validateParameters = function (func, args, types) {
    ...
    var symbol = 'X';
    if (diff > 0) {
      var givenX = args.length > 0 ? symbol + (',' + symbol).repeat(args.length - 1) : '';
      var correctX = args.length > 0 ? symbol + (',' + symbol).repeat(types[tindex].length - 1) : '';
      message = languageStrings[userLang].wrong_param_count.replace(/__FUNC__/g, func).replace(/__GIVEN_X__/g, givenX).replace(/__CORRECT_X__/g, correctX).replace(/__PARAMCOUNT__/g, types[tindex].length);
      if (types.length > 1) {
        message += languageStrings[userLang].multi_format.replace(/__FUNC__/g, func);
      }
    ...
    for (var format = 0; format < types.length; format++) {
      for (var p = 0; p < types[format].length && p < args.length; p++) {
        ...
        if ('undefined' === argType || null === argType) {
          message = languageStrings[userLang].empty_var.replace(/__FUNC__/g, func).replace(/__SPOT__/g, p + 1);
          report(message, func, EMPTY_VAR);
        } else if (!typeMatches(defType, argType, args[p])) {
          var received = 'string' === argType ? '"' + args[p] + '"' : args[p];
          message = languageStrings[userLang].empty_var.replace(/__FUNC__/g, func).replace(/__SPOT__/g, p + 1).replace(/__TYPE__/g, defType.toLowerCase()).replace(/__VAR__/g, received);
          if (types.length > 1) {
            message += languageStrings[userLang].multi_format.replace(/__FUNC__/g, func);
          }
